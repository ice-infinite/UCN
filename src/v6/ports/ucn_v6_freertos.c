...
struct ucn_v6_freertos_port {
    ...
    ucn_v6_protocol_owner_storage_t owner_storage;
    ...
    void *context, ucn_v6_owner_event_t event, bool from_isr)
{
    ...
    return ucn_v6_protocol_owner_post(port->owner, event, from_isr);
}

static ucn_v6_result_t dispatch_event(
    void *context, ucn_v6_owner_event_t event)
{
    ucn_v6_freertos_port_t *port = (ucn_v6_freertos_port_t *)context;
    uint64_t now_us;
    ucn_v6_result_t result;
    if (!port_valid(port) || port->adapter == NULL) return UCN_V6_ERR_STATE;
    result = port->ops.read_monotonic_time_us(port->ops.context, &now_us);
    if (result != UCN_V6_OK) return result;
    return port->handler(port->handler_context, event, now_us, port->adapter);
}
    ...
    ucn_v6_freertos_port_t **port_out)
{
    ...
    if (port_out == NULL || !ops_valid(ops) || handler == NULL ||
    ...
    port->handler_context = handler_context;
    port->canary = UCN_V6_FREERTOS_CANARY;
...
ucn_v6_result_t ucn_v6_freertos_port_run(
    ...
    uint16_t *processed)
{
    if (!port_valid(port) || processed == NULL || budget == 0U) {
        return UCN_V6_ERR_ARGUMENT;
    }
    if (port->adapter == NULL) return UCN_V6_ERR_STATE;
    return ucn_v6_protocol_owner_run(port->owner, budget, dispatch_event,
                                     port, processed);
}
    ...
    uint16_t *processed)
{
    ...
    result = ucn_v6_protocol_owner_copy_view(port->owner, &view);
    if (result != UCN_V6_OK) return result;
    if (view.pending_total == 0U) {
        result = port->ops.wait_for_notification(
            port->ops.context, max_wait_us, &notified);
        if (result != UCN_V6_OK) return result;
        if (!notified) {
            result = ucn_v6_protocol_owner_post(
                port->owner, UCN_V6_OWNER_EVENT_TIMER, false);
            if (result != UCN_V6_OK) return result;
        }
    }
    return ucn_v6_freertos_port_run(port, budget, processed);
}
    ...
    bool from_isr)
{
    ...
    return ucn_v6_protocol_owner_post(
