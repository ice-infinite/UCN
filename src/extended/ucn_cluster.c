...
static const uint32_t CLUSTER_TRANSITION_DIRECT_ALLOWED[UCN_CLUSTER_PHASE_COUNT]
    ...
    [UCN_CLUSTER_PHASE_DETACHED_OBSERVE] =
        ...
        /* joins a recovery Head: handle_recovery_declare() L3194 (role=MEMBER L3260) */
    ...
    [UCN_CLUSTER_PHASE_MEMBER_TAKEOVER_GRACE] =
        /* Head lease renewed: consider_head_offer() L2176 refresh (grace=0 L2193) /
         * handle_head_takeover() L3012 (grace=0 L3047) */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_MEMBER_ACTIVE) |
    ...
    [UCN_CLUSTER_PHASE_HEAD_BACKUP_ASSIGNING] =
        ...
        (UINT32_C(1) << UCN_CLUSTER_PHASE_HEAD_STABLE) |
        /* Backup lost: remove_member() L1961 (node_id=0 L1969) / expire_members() L3983 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_HEAD_NO_BACKUP) |
    ...
    [UCN_CLUSTER_PHASE_BACKUP_SYNCING] =
        /* snapshot READY: handle_backup_member_sync() L2566 SYNC_END (syncing=false L2642, ready=true L2643) */
        ...
        (UINT32_C(1) << UCN_CLUSTER_PHASE_ELECTION) |
        /* newer-Term Head: consider_head_offer() L2176 (takeover=false L2226) + backup_clear_sync() L2386 + begin_join() L1904 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_JOIN_PENDING) |
        /* HEAD_TAKEOVER / recovery: handle_head_takeover() L3012 (role=MEMBER L3040) */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_MEMBER_ACTIVE),

    [UCN_CLUSTER_PHASE_BACKUP_READY] =
        /* Primary lease lapsed: start_takeover() L2825 (takeover=true L2830) */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_BACKUP_TAKEOVER) |
        /* DELTA gap / resync: handle_backup_member_sync() L2566 (DELTA gap L2595-2596, SYNC_BEGIN L2617-2618) */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_BACKUP_SYNCING) |
        /* score challenge: backup_challenge() L2153 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_ELECTION) |
        /* newer-Term Head: consider_head_offer() L2176 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_JOIN_PENDING) |
        /* Primary lost / stepdown: HEAD_STEPDOWN -> backup_clear_sync() L2386 -> set_detached() L1875 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_DETACHED_OBSERVE) |
        /* HEAD_TAKEOVER / recovery: handle_head_takeover() L3012 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_MEMBER_ACTIVE),
    ...
    [UCN_CLUSTER_PHASE_RECOVERY_OBSERVE] =
        ...
        (UINT32_C(1) << UCN_CLUSTER_PHASE_JOIN_PENDING) |
        /* recovery Head join: handle_recovery_declare() L3194 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_MEMBER_ACTIVE),

    [UCN_CLUSTER_PHASE_RECOVERY_ELECTION] =
        /* quorum, declare: declare_recovery_head() L3155 (role=RECOVERY_HEAD L3160) */
    ...
    [UCN_CLUSTER_PHASE_RECOVERY_HEAD] =
        /* TTL expired (cooldown): stepdown_recovery_head() L3178 -> set_detached() L1875 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_RECOVERY_OBSERVE) |
        /* lost arbitration / HEAD_TAKEOVER: handle_recovery_declare() L3194 /
         * handle_head_takeover() L3012 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_MEMBER_ACTIVE) |
        /* stable Head reclaims: begin_ordered_stepdown() L2131 */
        (UINT32_C(1) << UCN_CLUSTER_PHASE_STEPPING_DOWN),
};
                                            ...
                                            uint32_t now_ms)
{
    ...
    case UCN_CLUSTER_PHASE_ELECTION:
        ...
        break;
    case UCN_CLUSTER_PHASE_JOIN_PENDING:
        /* begin_join() L1904: role=JOIN_PENDING L1912, abandons Recovery
         * candidacy L1910-1911; the mirror/known_backup are NOT cleared by
    ...
    case UCN_CLUSTER_PHASE_HEAD_STABLE:
         ...
         * L2562 / complete_election caller state). */
        cluster->role = UCN_CLUSTER_ROLE_HEAD;
        break;
    case UCN_CLUSTER_PHASE_BACKUP_SYNCING:
        /* handle_backup_assign() L2468 (role=BACKUP L2504, syncing=true
         * L2511, ready=false L2512) and handle_backup_member_sync() L2566
         * BEGIN/gap (L2617-2618, L2630-2631) both write role+syncing+ready;
        ...
        break;
    case UCN_CLUSTER_PHASE_BACKUP_READY:
        /* handle_backup_member_sync() L2566 SYNC_END: role=BACKUP,
         * syncing=false L2642, ready=true L2643. */
        cluster->role = UCN_CLUSTER_ROLE_BACKUP;
        cluster->backup_ready = true;
        cluster->backup_syncing = false;
        break;
    case UCN_CLUSTER_PHASE_BACKUP_TAKEOVER:
        /* start_takeover() L2825: role=BACKUP, takeover=true L2830;
        ...
        break;
    case UCN_CLUSTER_PHASE_STEPPING_DOWN:
        /* begin_ordered_stepdown() L2131: role=STEPPING_DOWN L2139, yields
         * Recovery candidacy L2136-2137; the Head keeps its Backup
