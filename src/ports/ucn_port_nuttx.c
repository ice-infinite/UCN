...
/*
 * EN: Initializes the NuttX Port object from validated caller-owned configuration without heap allocation.
 * 中文：使用经验证的调用方配置初始化 NuttX Port 对象，且不使用堆内存。
 */
    ...
    const ucn_nuttx_port_config_t *config)
{
    ...
    if (port == NULL || config == NULL || config->ops == NULL ||
        config->ops->notify_protocol_worker == NULL ||
...
/*
 * EN: Copies `rx_enqueue` into a bounded NuttX Port queue.
 * 中文：把 `rx_enqueue` 复制到固定容量的 NuttX Port 队列。
 */
    ...
    bool from_isr)
{
    ...
    port->ops->notify_protocol_worker(port->runtime_context, from_isr);
...
/*
 * EN: Advances one bounded `worker_step` state-machine step in NuttX Port.
 * 中文：在 NuttX Port 中推进一次有界的 `worker_step` 状态机步骤。
 */
...
/*
 * EN: Waits for `worker_wait` using the scheduler contract of NuttX Port.
 * 中文：按照 NuttX Port 的调度合同等待 `worker_wait`。
 */
...
/*
 * EN: Returns the current `stats` view from NuttX Port state.
 * 中文：从 NuttX Port 状态返回当前 `stats` 视图。
 */
...
/*
 * EN: Returns the current `runtime_stats` view from NuttX Port state.
 * 中文：从 NuttX Port 状态返回当前 `runtime_stats` 视图。
 */
