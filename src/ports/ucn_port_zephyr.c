...
/*
 * EN: Initializes the Zephyr Port object from validated caller-owned configuration without heap allocation.
 * 中文：使用经验证的调用方配置初始化 Zephyr Port 对象，且不使用堆内存。
 */
ucn_result_t ucn_zephyr_port_init(
    ucn_zephyr_port_t *port,
    const ucn_zephyr_port_config_t *config)
{
    ucn_result_t result;
...
/*
 * EN: Copies `rx_enqueue` into a bounded Zephyr Port queue.
 * 中文：把 `rx_enqueue` 复制到固定容量的 Zephyr Port 队列。
 */
ucn_result_t ucn_zephyr_port_rx_enqueue(
    ucn_zephyr_port_t *port,
...
/*
 * EN: Advances one bounded `thread_step` state-machine step in Zephyr Port.
 * 中文：在 Zephyr Port 中推进一次有界的 `thread_step` 状态机步骤。
 */
ucn_result_t ucn_zephyr_port_thread_step(
    ucn_zephyr_port_t *port,
    size_t *pumped,
    uint8_t *bridged)
{
...
/*
 * EN: Waits for `thread_wait` using the scheduler contract of Zephyr Port.
 * 中文：按照 Zephyr Port 的调度合同等待 `thread_wait`。
 */
ucn_result_t ucn_zephyr_port_thread_wait(
    ucn_zephyr_port_t *port,
...
/*
 * EN: Returns the current `stats` view from Zephyr Port state.
 * 中文：从 Zephyr Port 状态返回当前 `stats` 视图。
 */
ucn_zephyr_port_get_stats(const ucn_zephyr_port_t *port)
{
...
/*
 * EN: Returns the current `runtime_stats` view from Zephyr Port state.
 * 中文：从 Zephyr Port 状态返回当前 `runtime_stats` 视图。
 */
ucn_zephyr_port_get_runtime_stats(const ucn_zephyr_port_t *port)
{
