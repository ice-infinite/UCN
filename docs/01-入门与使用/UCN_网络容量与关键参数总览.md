...
> `ARCHIVED / NOT CURRENT`：本文保存 v5 容量记录；当前 v6 资源与参数请见 [v6 资源、性能与参数调优](../用户手册/05-故障诊断与产品验收.md)。
...
完整证据：[S21测试方案](../08-实现与验证/版本演进/UCN_S21_极限规模模拟测试方案.md)、[S21测试结果](../08-实现与验证/版本演进/UCN_S21_极限规模模拟测试结果.md)、[逐节点与汇总CSV](../results/S21/)。
...
两跳同时冷启动时，W0/mixed 的 254 Node 交付率为 15.354%，W1/W2/W3 的 4096 Node 都为 11.377%，不同档位的交付数完全一致，瓶颈来自默认 Route/Discovery/Pending 固定工作集而不是 Codec。丢包、重复、延迟、断链和 Q0/Q1 组合下五种布局均通过正确性门禁。完整证据见 [V5-10 单档与混档极限模拟报告](../08-实现与验证/版本演进/UCN_V5_10_单档与混档极限模拟报告.md) 和 [V5-10 汇总 CSV](../results/V5_10/V5_10_extreme_summary.csv)。
...
完整实现与验证见[S22 稳定化修复报告](../08-实现与验证/版本演进/UCN_S22_重复抑制与稳定化修复报告.md)。
