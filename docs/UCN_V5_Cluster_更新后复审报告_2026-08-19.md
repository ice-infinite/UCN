...
| 项目 | 文件/位置 | 说明 |
|---|---|---|
| token bucket 冷启动补偿 | [src/extended/ucn_cluster.c](E:/File/MESH/UCN/src/extended/ucn_cluster.c:1566) | `token_bucket_refill()` 增加了 `last_refill_ms==0` 的冷启动分支，避免启动即触发“虚假的满量补发”导致速率失真。 |
| 邻居列表同步原子化 | [src/extended/ucn_cluster.c](E:/File/MESH/UCN/src/extended/ucn_cluster.c:1858) | `ucn_cluster_sync_neighbors()` 改为先 stage 后 memcpy，避免溢出中途留下半写状态。 |
| `RECOVERY_HEAD -> MEMBER_ACTIVE` 迁移 | [src/extended/ucn_cluster.c](E:/File/MESH/UCN/src/extended/ucn_cluster.c:3911) | `handle_head_takeover()` 的 Recovery Head 入站边先做 transition 后做 site-write，且失败时 fail-closed。 |
| 备份接管回放/状态更新顺序 | [src/extended/ucn_cluster.c](E:/File/MESH/UCN/src/extended/ucn_cluster.c:2986) | `handle_backup_assign()` 的主链路已增加明显的角色路径约束与 post-commit derive 断言，降低了回放重入窗口的非一致行为。 |
| 成员/备份位宽边界约束 | [include/ucn/ucn_cluster.h](E:/File/MESH/UCN/include/ucn/ucn_cluster.h:149) | 在头文件加入 `UCN_CLUSTER_MAX_MEMBERS <= 32` 的静态断言，规避位移 UB。 |
...
- **文件**：[src/extended/ucn_cluster.c](E:/File/MESH/UCN/src/extended/ucn_cluster.c:3012)
...
### P1-3: 跨簇缓存写入缺少时序幂等保护
- **文件**：[src/extended/ucn_cluster_federation.c](E:/File/MESH/UCN/src/extended/ucn_cluster_federation.c:933)
...
### P2-1: MSVC 构建阻塞（可编译性）
- **文件/错误**：[src/extended/ucn_cluster.c:1049](E:/File/MESH/UCN/src/extended/ucn_cluster.c:1049)
