...
当前执行结果：S04 已按上述边界完成代码和 Host 软件验证。Nano 使用独立静态 Node，Lite 在预处理/构建层移除 Candidate、Path、Policy 和 Diagnostic，Service 可独立关闭；三档功能矩阵、当前最小帧 33/46/64 B、Full 回归及 GCC 严格构建均已通过。S08 又将 `ucn_node.h` 收口为 API/不完整类型，以 `ucn_node_storage.h` 让唯一 owner 显式取得静态布局，并明确 Seen Cache 不等价于生产 Replay Window。详细证据见 [S04 Feature Profile 与资源报告](../../08-实现与验证/版本演进/UCN_S04_Feature_Profile与资源报告.md) 和 [S08 公共 API 与静态存储边界](../../08-实现与验证/版本演进/UCN_S08_公共API与静态存储边界.md)。Host 结果不能替代 ESP32/STM32 ELF、栈/Heap 或真实介质测试，后者继续归 S06/S07。
...
V5-44/V5-36 后 T22.4 当前选择规则统一写成：

```text
LC-1 effective_select_cost ×（当前活动 Flow 数 + 1）
```

Unknown 不优于 Known；RTT、失败率、Queue 和介质指标只通过 LC-1 固定整数表归一后进入本地分数，禁止裸加；持续 Adapter Queue Pressure 仍可作为未到期 Flow 的拥塞重绑触发条件。项目任务表和知识库专题必须同步修正，历史操作记录保留原文并由新记录注明已被当前实现取代。
...
S05 不修改 MCU 对象、队列容量或线格式；1024 槽事件队列和观测表只存在于 Host 测试程序。虚拟结果不能替代 ESP-NOW/UART/CAN 的真实碰撞、时延、栈/Heap、功耗或三板多跳，后者继续属于 S06/S07。复现命令与完整边界见 [S05 动态综合压力测试](../../08-实现与验证/版本演进/UCN_S05_动态综合压力测试.md)。
