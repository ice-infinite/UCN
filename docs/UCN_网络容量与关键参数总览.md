...
> 版本口径：UCN v4，仓库 `main@e0864a9`，2026-08-11。
...
1. **地址空间**：单个 Network ID 内可使用 4,294,967,294 个 Node ID。
...
4. **已验证规模**：Host 稀疏 Tree 的直连业务验证到 4096 Node；高并发全对全场景只有 8 Node 保持 100%，16 Node 已出现固定表/建路工作集饱和。实机目前只有两块 ESP32-S3 的一跳台架证据。

因此当前最准确的产品口径是：

> UCN 使用 32 位 Node ID，当前默认每节点支持 4 个逻辑 Link、8 个 Neighbor 状态、每 Neighbor 2 个 Bearer、8 条远端 Route、4 个并发寻路和最多 16 跳；总网节点数由拓扑、活跃目的地、共享介质与负载共同决定，不能由 4096 Node 稀疏模拟直接外推。
...
- 每个产品在 `ucn_config_t.default_hop_limit` 中配置实际值，合法范围为 `1..16`。
- 最长业务路径可经过 16 条 Link。
...
- RREQ：32 B 头 + 16 B Payload = 48 B/扩散 Link；
- RREP：32 B 头 + 18 B Payload = 50 B/返回 Hop；
- Q0 永不等待或主动发起 RREQ，必须预先具备直连、静态 Route、缓存 Route 或安装好的 Path。

一次冷寻路的 UCN 线字节可粗略表示为：

```text
48 B × RREQ实际扩散次数 + 50 B × 返回跳数
```
...
默认最大帧 256 B，CRC 已包含在头部：

| 帧形态 | 头部 | E2E Tag | 最大 Payload | 满帧Payload效率 |
| --- | ---: | ---: | ---: | ---: |
| 普通明文 | 32 B | 0 | 224 B | 87.500% |
| Route明文 | 36 B | 0 | 220 B | 85.938% |
| Path明文 | 40 B | 0 | 216 B | 84.375% |
| 普通受保护 | 32 B | 16 B | 208 B | 81.250% |
| Route受保护 | 36 B | 16 B | 204 B | 79.688% |
| Path受保护 | 40 B | 16 B | 200 B | 78.125% |

典型小帧：

- 16 B普通明文单跳理论效率：`16/(32+16)=33.333%`；S21包含控制量后约31.25%。
- 32 B普通明文单跳理论效率：50%；S21高负载约49.1%～49.5%。
...
| --- | ---: | ---: |
| Nano | 2,648 B | 13,132 B |
| Lite | 5,888 B | 52,120 B |
| Full | 9,400 B | 109,900 B |

这是 S22 Source/Session 位图窗口与独立 RREQ Cache 完成后的当前值。相对 S22 前基线，Nano/Lite/Full Node 分别变化 `+8/+432/+1264 B`；具体原因、`.text` 变化和重放结果见[S22 稳定化修复报告](UCN_S22_重复抑制与稳定化修复报告.md)。
