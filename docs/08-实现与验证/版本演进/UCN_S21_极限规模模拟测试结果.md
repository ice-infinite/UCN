...
本报告保存的是 S21 初始固定 W3 证据；V5-10 已在同一 Harness 上补齐 W0/W1/W2/W3 单档与 mixed 混档矩阵、逐档 CSV、地址域负向门禁和 HELLO 最大接收档声明修复。新增结论以 [V5-10 单档与混档极限模拟报告](UCN_V5_10_单档与混档极限模拟报告.md) 为准，历史场景和已暴露过的问题仍保留在本文。
...
> 后续状态（2026-08-11）：S22 已完成上述重构；相同 M1/M2/M4 重放均为 PASS、重复业务 0。此处保留旧 FAIL 作为缺陷复现证据，修复后的源码、资源与 CSV 见[S22～S26 稳定化修复报告](UCN_S22_重复抑制与稳定化修复报告.md)和[`results/S22`](../../results/S22/)。
