...
> 关联：[整体架构设计](../../02-总体架构/UCN_整体架构设计.md) · [协议分层与配置档案](../../01-入门与使用/UCN_协议分层与配置档案.md) · [Adapter 契约](../../06-平台与适配/UCN_Adapter_契约.md) · [v4 节点快照诊断](UCN_v4_节点快照诊断.md) · [任务表](../../00-项目管理/00-任务表.md)
...
同一 MCU 内的业务任务现在可调用 Core 外的 `ucn_service_send()`：发给本机 Node 时直接投入静态 Inbox，发给远端 Node 时先进入固定 Remote TX 队列。T25.2 的 `ucn_service_protocol_bridge_step()` 只能由唯一 Protocol Task 有界出队并调用 `ucn_node_send_endpoint()`，目标 Endpoint callback 再投递 Router；T25.3 才会映射 FreeRTOS 静态 Queue/通知。因此不能把当前实现误认为完整的板级任务通信；详见[节点内任务通信建议](../../05-传输与服务/UCN_节点内任务通信建议.md)。
