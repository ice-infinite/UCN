...
> v5 迁移说明（2026-08-11）：本文主体保留为 `v4.0.0-final-before-v5` 的完整设计快照，其中 32/36/40 B、版本 4 和逐字段偏移不再是当前 v5 线格式。当前实现请先读 [v5 Adaptive Wire Profile 设计方案](../08-实现与验证/版本演进/UCN_v5_Adaptive_Wire_Profile设计方案.md)、[V5-14 Cost/RREQ/RREP 报告](../08-实现与验证/版本演进/UCN_V5_14_长距离Cost与RREQ_RREP实现报告.md) 与 [V5-15 控制载荷报告](../08-实现与验证/版本演进/UCN_V5_15_Profile感知控制载荷实现报告.md)；路由、Service 和产品安全边界在未被 v5 专题替代前继续参考本文。
...
> 关联资料：[整体架构设计](UCN_整体架构设计.md) · [协议分层与配置档案](../01-入门与使用/UCN_协议分层与配置档案.md) · [路由缓存与自动选路建议](../04-路由与链路/UCN_路由缓存与自动选路建议.md) · [节点入离网与网络稳定性建议](../04-路由与链路/UCN_节点入离网与网络稳定性建议.md) · [任务表](../00-项目管理/00-任务表.md)
...
透明中继不能修改端到端受保护的来源、目的、Endpoint、Session、Sequence、业务密文等不可变数据；但允许按协议改写 Hop Limit、路径扩展和 CRC。v4 Core 已提供固定 AAD、Tag 和 `seal/open` Provider 契约；可变/不可变字段、生产密钥和实际 AEAD 见[按需加密与透明转发建议](../03-核心协议/安全/UCN_按需加密与透明转发建议.md)。
...
3. 生产级身份、经审计 AEAD、密钥轮换与完整 ACL 策略；v4 Core 已有[按需加密与透明转发建议](../03-核心协议/安全/UCN_按需加密与透明转发建议.md)中的帧/Provider/策略基础，产品密钥和实机测试仍待落地。
