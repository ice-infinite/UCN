...
> 关联：[T21 同对端多介质主备 Link](00-任务表.md#任务清单) → [T22 策略路由与可选负载均衡](00-任务表.md#任务清单)。
...
自动均衡采用“流亲和 + 租约 + 有界重绑定”：同一 `(source, destination, endpoint[, stream_id])` 在租约内固定 Path；仅在流创建、租约到期、Path Down 或持续拥塞时重新选择。初版可用平滑 Cost、队列压力、RTT 与失败率生成权重，但不复制同一帧，不做带宽聚合。
...
首次发送或租约到期时，Core 用固定 Flow 表绑定一条成员 Path；`balance_flow_lease_ms=0` 使用默认 2 s。分数为可用的平滑 Cost、RTT、失败率、队列压力之和，再乘以该 Path 当前活跃 Flow 数加一，因此质量相同的两个新 Flow 会自然分到不同 Path，而明显更优的 Path 可按比例承担更多 Flow。业务帧本身不添加字段。
