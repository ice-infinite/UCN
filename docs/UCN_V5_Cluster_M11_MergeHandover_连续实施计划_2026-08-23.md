...
现场依赖仍有 M05 `AUDIT HOLD`、M08 `WAIT EXTERNAL` 和 M10 `WAIT EXTERNAL RE-REVIEW`。因此 M11 的完整协议模型必须是 **caller-owned、default-OFF 的实验 archive**，不能把 RFC4 Type 26..29 编解码、发送、RX、Authority 或 Adapter 接入默认产品。本计划已获 M11 **外部复审 GO（受限实验范围）**；该签字不改变上述依赖或默认产品边界。
