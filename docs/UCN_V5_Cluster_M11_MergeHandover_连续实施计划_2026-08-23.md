...
| 11-02 | Candidate state | **PASS（R08-B 后）**：live candidate 与固定容量 replay tombstone 分层保存 foreign identity、score samples、cluster size/capacity、wire offer、Config、tenure/hold-down/replay；expiry 只失活资格，不能时间淘汰 history。 | stale/replay/过期均无写；tombstone 满时不驱逐 history，新身份 `UCN_ERR_NO_SPACE`。 |
...
12. candidate 的 activity expiry 只能结束当前连续资格，不能删除 Epoch/Config nonce high-water 或已 armed 的 hold-down。history 必须转入静态、无时间淘汰的 tombstone；没有可证明旧 offer 不可重放的 incarnation/freshness 证据前，容量满只能拒绝新 admission，不能回收 history。
