...
> | M03 Epoch 分类 | AUDIT HOLD / R10 SELF-AUDIT PASS / WAIT EXTERNAL RE-REVIEW | 03-01..03-09 + R01..R10 | R10 已以 Epoch comparator 收敛 JOIN_PENDING 重定向：foreign 不读 Term，同簇只按分类处理；Windows Debug/Release、WSL ASan/UBSan 与生产 RX 定向回归均通过。其余已签字范围不回退，须外部复审后才可恢复 M03 完整签字。 |
...
> | M11 Merge/Handover | AUDIT HOLD / R07–R08 SELF-AUDIT PASS / WAIT EXTERNAL RE-REVIEW（受限实验范围） | R01–R06-B 已闭环；R07/R08 已收紧为连续合格样本与完整 proposal/replay 域，并经当前矩阵自审通过，等待重新外审 | 仍仅限 caller-owned、default-OFF 实验模型。M05 顶层 `AUDIT HOLD`、M08 `WAIT EXTERNAL` 与 M10 外审等待均不解除；不得把 M11 接入 production v4 RX/TX/FSM、Authority、Adapter 或实机结论。 |
...
| `CLV2-03-R10` | P0 | **SELF-AUDIT PASS / WAIT EXTERNAL RE-REVIEW** | **JOIN_PENDING Epoch-domain residual closure**：重定向已构造 pending/remote Epoch 并只使用 `ucn_cluster_epoch_compare()`。FOREIGN 统一走显式 foreign retarget 策略、不读取/比较 Term；同簇只接受 remote `LOWER`（较新）或 `CONFLICT`（不同 Head 同 Term）重定向，`HIGHER`（远端陈旧）和 `SAME` 均 no-write。 | 生产 RX 回归固定 `A/100/H → B/1/H` 与 `A/1/H → B/100/H` 为相同 foreign retarget 结果；`A/2/H → A/3/H` 重定向，`A/3/H → A/2/H` 保持完整 pending 状态；源码扫描不再有 JOIN_PENDING 裸 Term 比较。 |
...
| 任务 ID | 优先级 | 依赖 | 主要文件/位置 | 具体修改任务 | 完成定义 / 测试 |
|---|---:|---|---|---|---|
| `CLV2-07-00` | P2 | M05、M04 R23 | Record schema / migration | **DONE / 外审并入 M07 final；M10 已 supersede 当前 writer 为 schema v3**：M07 当时将 writer 升级为 schema v2，v1 仅只读兼容，decode provenance 保留在 `record_schema_version`。M10 现以 append-only 的 v3 取代 writer；v1/v2 均只读，`LEGACY_PREPARED_ABORT` 的历史语义仍只针对 v1。 | 历史 v1/v2 provenance 与 legacy-abort 回归保留；M10 另覆盖 v1/v2 read-only decode、v3 292 B writer 与 partial Vote 禁止作为 M10 proof。见 `UCN_V5_Cluster_M07_00_RecordV2Provenance_自审报告_2026-08-23.md` 与 M10 全量自审报告。 |
...
| `CLV2-10-02` | P0 | 10-01 | VoteId | **CODE COMPLETE / SELF-AUDIT PASS**：VoteId 为 `{cluster,old_term,proposed_term,config_id,backup_id,generation,snapshot_id}`；schema v3 292 B append-only 持久化完整身份，v1/v2 partial Vote 永不作为 M10 proof。 | 同 Term 的 config/snapshot 冲突、CRC 损坏、partial v1/v2 均拒绝。 |
...
**状态：** **AUDIT HOLD / R07–R08 SELF-AUDIT PASS / WAIT EXTERNAL RE-REVIEW（受控实验范围）**。R01–R06-B 已获受限实验范围外审 GO；R07 已令 `score_samples` 只计同一 local score、improvement/required-samples/required-capabilities 输入下连续达到 threshold 的 fresh offer，R08 已把 proposal/replay 域扩展为 full Epoch、Config、wire/capabilities、Backup-policy compatibility。当前自审通过后须重新外审。完整计划、分项/全量自审和外部审计入口见 `UCN_V5_Cluster_M11_MergeHandover_连续实施计划_2026-08-23.md`、`UCN_V5_Cluster_M11_分项与全量自审报告_2026-08-23.md`、`UCN_V5_Cluster_M11_外部审计材料_2026-08-23.md`。M11 仍不得进入 M12。
...
| `CLV2-11-10` | P0 | 11-01..09 | Merge suite | **SELF-AUDIT PASS / WAIT EXTERNAL RE-REVIEW（受限实验范围）**：组合覆盖双簇 capacity、抖动、retry/replay、赢家/撤权后旧 Head故障、Backup/Provisional 和 Stepdown，并新增 RFC typed 对照、duration/serial、target commit deadline/no-fake-durable、transaction corruption、撤权 transaction 的 Begin/Reset→Begin，以及 R07/R08 的连续合格样本/完整 proposal domain 反例。 | 全量矩阵与边界扫描通过；结果仅限实验软件模型，须重新外审。 |
...
| `CLV2-11-R07` | P1 | 11-02、11-03 | Consecutive qualifying score samples | **SELF-AUDIT PASS / WAIT EXTERNAL RE-REVIEW**：`score_samples` 只计入同一 proposal/domain、同一 local score、improvement percent、required samples 与 required capabilities 输入下连续达到 improvement threshold 的 fresh offer；不合格 score 或这些 qualification 输入改变都会清零，不能累计普通 fresh packet。eligible 判定也必须精确匹配当前 inputs。 | 固定 `870→890`（required=2）仍不可 eligible；`890→870` 清零；`890→870→890→890` 仅最后一帧 eligible；阈值边界明确为 `>=`；已累积后本地 score/qualification 输入改变也立即不可 eligible。 |
| `CLV2-11-R08` | P1 | 11-02、R07 | Candidate proposal/replay domain | **SELF-AUDIT PASS / WAIT EXTERNAL RE-REVIEW**：candidate proposal/replay 域精确绑定 full Epoch、Config、cluster size/capacity、wire/capabilities 与 Backup-policy compatibility；local score/qualification inputs 则独立重置连续样本但保留同 proposal 的 nonce replay gate。offer proposal 域任一变化建立新 proposal，重置 nonce replay history、连续样本和 first-seen；同 proposal 域才要求 nonce 严格递增。 | `B/T100/C10/nonce100 → B/T101/C11/nonce1` 必须作为新 proposal 接纳；Config/capability 变化不继承样本；同域较小/相同 nonce 仍 `REPLAY` 且表不写。 |
