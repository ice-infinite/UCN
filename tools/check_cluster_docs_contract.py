...
def main() -> None:
    ...
    contract = (root / "docs/07-Cluster簇/00-规范与状态机/UCN_Cluster_Code_Doc_Contract.md").read_text(encoding="utf-8")
    current = (root / "docs/07-Cluster簇/00-规范与状态机/UCN_V5_Cluster_CURRENT_FSM.md").read_text(encoding="utf-8")
    target = (root / "docs/07-Cluster簇/00-规范与状态机/UCN_V5_Cluster_FSM_Design_v2.md").read_text(encoding="utf-8")
    rfc = (root / "docs/07-Cluster簇/00-规范与状态机/UCN_Cluster_Wire_v4.md").read_text(encoding="utf-8")
    tasks = (root / "docs/07-Cluster簇/00-规范与状态机/UCN_V5_Cluster_Current_to_Target_v2_详细修改方案与任务表.md").read_text(encoding="utf-8")
